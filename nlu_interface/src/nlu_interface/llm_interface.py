...
class Prompt:
    ...
    def to_openai_messages(self, num_incontext_examples):
        ...
        user_message_content += self.new_instruction_preamble.format(
            response_format=self.response_format
        )
...
class LLMInterface(ABC):
    ...
    def __init__(
        ...
        if api_key_env_var:
            ...
            logger.warning(
                "No API key environment variable provided, using OPENAI_API_KEY as a default."
            )
    ...
    def _validate_initialization(self):
        raise NotImplementedError(
            'Subclasses must implement "_validate_initialization()".'
        )
...
class OpenAIWrapper(LLMInterface):
    ...
    def _validate_initialization(self):
        if self.model not in self.valid_model_names:
            raise ConstructionFailure(
                f'The model "{self.model}" was not recognized (valid models: {str(self.valid_model_names)}).'
            )
        if self.mode not in self.valid_prompt_modes:
            raise ConstructionFailure(
                f'The prompt mode "{self.mode}" was not recognized (valid modes: {str(self.valid_prompt_modes)}).'
            )
        if self.num_incontext_examples < len(self.prompt.incontext_examples):
            raise ConstructionFailure(
                f"The requested number of incontext examples ({self.num_incontext_examples}) is greater than the number available ({len(self.prompt.incontext_examples)})."
            )
        if self.debug:
    ...
    def _create_client(self):
        ...
        if self.debug:
            logger.debug(
                f'Successfully created an OpenAI client for model "{self.model}".'
            )
        return
