...
class OpenAIConfig(LLMConfig):
    ...
    def resolve_api_key(self) -> str:
        if self.api_key_env_var:
            ...
            logger.warning(
                f'No API key env var provided; defaulting to "{self.api_key_env_var}".'
            )
            api_key = os.getenv(self.api_key_env_var)
        if not api_key:
...
class AnthropicBedrockConfig(LLMConfig):
    ...
    def resolve_access_key(self) -> str:
        if self.access_key_env_var:
            ...
            logger.warning(
                f'No Access key env var provided; defaulting to "{self.access_key_env_var}".'
            )
            access_key = os.getenv(self.access_key_env_var)
        if not access_key:
    ...
    def resolve_secret_key(self) -> str:
        if self.secret_key_env_var:
            ...
            logger.warning(
                f'No Secret key env var provided; defaulting to "{self.secret_key_env_var}".'
            )
            secret_key = os.getenv(self.secret_key_env_var)
        if not secret_key:
